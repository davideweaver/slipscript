...
char *get_POST()
{
  ...
  if (CONTENT_LENGTH != NULL) {
    ...
    if (fread(buffer,sizeof(char),content_length,stdin) != content_length) {
		/* consistency error. */
		/*html_error(NULL,"Input length < CONTENT_LENGTH.", EMAIL);*/
		/*exit(1);*/
		return NULL;
    }
    buffer[content_length] = '\0';
		...
		/*html_error(NULL,"CONTENT_LENGTH is null.", EMAIL);*/
		/*exit(1);*/
		return NULL;
  }
  return buffer;
...
char *get_GET()
{
  ...
  if (QUERY_STRING == NULL) {
	/*html_error(NULL,"QUERY_STRING is null.", EMAIL);*/
    /*exit(1);*/
	  return NULL;
  }
	...
	tempstr++;
      *tempstr = '\0';
      if ( (uploadfile = fopen(uploadfname,"w")) == NULL) {
			html_error(NULL,"Cannot upload file.", EMAIL);
			return 1;
      }
    }
    else
...
int read_cgi_input(llist* entries)
{
  ...
  if (REQUEST_METHOD == NULL)
	...
	html_error(NULL,"REQUEST_METHOD invalid.", EMAIL);
    return 1;
  }
  /* parse the input */
