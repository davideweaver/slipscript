...
static void do_define(CompilerContext *c);
...
static void do_function(CompilerContext *c,PVAL *pv);
...
static void do_prop_call(CompilerContext *c,PVAL *pv, char *obj);
...
static void do_statement(CompilerContext *c)
{
	...
	case T_FUNCTION: do_define(c); break;
...
static void define_method(CompilerContext *c)
{
	...
	/* step back line ptr */
	for( i=0; i<=(int)strlen(c->t_token)+1; i++ )
		c->linePtr--;
	c->savedChar = '\0';
    ...
    /* the first arguments are always 'self' and 'super' */
    AddArgument(c,c->arguments,"this");
    AddArgument(c,c->arguments,"super");
...
static void do_expr15(CompilerContext *c,PVAL *pv)
{
	...
	strcpy( obj, c->t_token );	/* store object name for method call, DW fix */
	...
	case '.':
		...
		if( c->savedChar == '(' ) do_prop_call(c,pv,obj);
	    break;
	case T_INC:
...
{
    switch (token(c)) {
    //case T_FUNCTION:		/* removed by me so function and define act the same way */
 	//do_function(c,pv);
	//break;
    case '\\':
	do_literal(c,pv);
...
/* do_prop_call - compile a function call */
static void do_prop_call(CompilerContext *c,PVAL *pv, char *obj)
{
    int tkn,n=0;
	...
	strcpy( c->t_token, obj );
	c->savedToken = 257;
	putcbyte(c,OP_PUSH);
	findvariable(c,c->t_token,pv); 
	fixup(c,0,codeaddr(c));
	fixup(c,0,codeaddr(c));
	rvalue(c,pv);
	//putcbyte(c,OP_DUP);
	++n;
...
static void do_send(CompilerContext *c,PVAL *pv)
{
	...
	//c->lbase[selLiteral] = InternCString(c->ic,selector);
