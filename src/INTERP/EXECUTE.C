...
static void Interpret(InterpreterContext *c)
{
	...
	case OP_GREF:
		...
		p3 = SymbolPrintName(p1);					/* my code */
		c->err = (char *)StringDataAddress(p3);		/* my code */
...
{
    CallErrorHandler(c,ecTooManyArguments,c->err);
}

void TooFewArguments(InterpreterContext *c)
{
    CallErrorHandler(c,ecTooFewArguments,c->err);
}
